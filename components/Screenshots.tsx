  ...
  return (
    <div className={`flex flex-col ${reverse ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-10 lg:gap-16`}>
      ...
      <div className="flex-1 text-center lg:text-left">
        <h3 className="text-2xl md:text-3xl font-black text-gray-900 mb-4">{title}</h3>
...
function VorratScreen() {
  return (
    <div className="w-full h-full bg-[#f8fafc] flex flex-col">
      <div className="px-3 pt-8 pb-2 bg-gradient-to-b from-[#3D6B8F] to-[#2A5470]">
        <p className="text-[10px] font-black text-white">Mein Vorrat</p>
        <p className="text-[8px] text-blue-100/70">32 Artikel · 3 laufen bald ab</p>
      </div>
      <div className="px-2 py-2 flex-1 space-y-1 overflow-hidden">
        {[
          ...
          <div key={item.n} className={`flex items-center gap-1.5 rounded-lg px-2 py-1 ${item.w ? 'bg-red-50 border border-red-200' : 'bg-white border border-gray-100'}`}>
            <span className="text-[12px]">{item.e}</span>
            <div className="flex-1">
              <p className="text-[8px] font-semibold text-gray-800">{item.n}</p>
              <p className={`text-[6px] ${item.w ? 'text-red-500' : 'text-gray-400'}`}>{item.d}</p>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="w-full h-full bg-[#f8fafc] flex flex-col">
      <div className="px-3 pt-8 pb-2 bg-gradient-to-b from-[#3D6B8F] to-[#2A5470]">
        ...
        <p className="text-[8px] text-blue-100/70">Aus deinem Vorrat generiert</p>
      </div>
      <div className="px-2 py-2 flex-1 space-y-1.5 overflow-hidden">
        {[
          ...
          <div key={r.n} className="bg-white rounded-xl p-2.5 border border-gray-100 shadow-sm">
            <div className="flex items-start justify-between mb-1">
              <p className="text-[9px] font-bold text-gray-800 flex-1">{r.n}</p>
              <span className="text-[10px]">{r.tag}</span>
            </div>
            <div className="flex gap-2">
              <span className="text-[7px] text-[#3D6B8F]">⏱ {r.t}</span>
              <span className="text-[7px] text-orange-500">🔥 {r.k}</span>
            </div>
          </div>
        ))}
        <div className="bg-gradient-to-r from-[#3D6B8F]/10 to-[#56B4A0]/10 border border-[#3D6B8F]/20 rounded-xl p-2.5">
          <p className="text-[8px] font-bold text-[#3D6B8F]">✨ Neu generieren</p>
          <p className="text-[7px] text-gray-400">Mit aktuellen Vorräten</p>
        </div>
      </div>
    </div>
  )
    ...
    <div className="w-full h-full bg-[#f8fafc] flex flex-col">
      ...
      <div className="px-2 py-2 flex-1 space-y-1.5 overflow-hidden">
        {[
          ...
          <div key={p.n} className="bg-white rounded-xl p-2 border border-gray-100 shadow-sm">
            <div className="flex items-center gap-1.5 mb-1">
              <span className="text-[12px]">{p.u}</span>
              <p className="text-[8px] font-bold text-gray-800">{p.n}</p>
            </div>
            <p className="text-[7px] text-gray-500 mb-1.5">{p.t}</p>
            <div className="bg-[#3D6B8F]/10 border border-[#3D6B8F]/20 rounded-lg px-2 py-1">
              <p className="text-[7px] text-[#3D6B8F]">{p.r}</p>
            </div>
            <p className="text-[6px] text-gray-400 mt-1">❤️ {p.l} Likes</p>
          </div>
        ))}
      </div>
    </div>
...
export default function Screenshots() {
  return (
    <section id="screenshots" className="bg-white py-28 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-20">
          ...
          <div className="mt-4 inline-flex items-center gap-2 bg-[#3D6B8F]/10 border border-[#3D6B8F]/20 text-[#3D6B8F] text-xs px-4 py-2 rounded-full">
